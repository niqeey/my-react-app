...
const DidNotStart = () => {
    ...
    useEffect(() => {
        const fetchDidNotStartParticipants = async () => {
            try {
                console.log('Fetching DNS participants for eventId:', eventId);
                const response = await authFetch(`${apiBase}/statistic/dns`, {
                    method: 'POST',
                ...
                console.error('Error fetching Did Not Start participants:', error);
            } finally {
                setLoading(false);
            }
        };
        ...
        if (eventId) {
            fetchDidNotStartParticipants(); // Call the function only if eventId is available
        }
    ...
    return (
        <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 16 }}>
                <h2>Did Not Start Participants</h2>
                <button
                    onClick={async () => {
                        try {
                            const res = await authFetch(`${apiBase}/report/event/dns/xlsx`, {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                            ...
                            a.href = url;
                            a.download = `${eventName || 'event'}_did_not_start.xlsx`;
