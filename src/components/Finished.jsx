...
const Finished = () => {
    ...
    useEffect(() => {
        const fetchFinishedParticipants = async () => {
            try {
                console.log('Fetching finished participants for eventId:', eventId);
                const response = await authFetch(`${apiBase}/statistic/finished`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ eventId }) // Use eventId from the URL
                });
                const result = await response.json();
                console.log('Finished response:', result);
                console.log('Is array?', Array.isArray(result), 'Length:', result?.length);
                setData(result);
            } catch (error) {
                console.error('Error fetching finished participants:', error);
            } finally {
                setLoading(false);
            }
        };
        ...
        if (eventId) {
            fetchFinishedParticipants(); // Call the function only if eventId is available
        }
        ...
        <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 16 }}>
                ...
                <button
                    onClick={async () => {
                        try {
                            const res = await authFetch(`${apiBase}/report/event/finished/xlsx`, {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                            ...
                            a.href = url;
                            a.download = `${eventName || 'event'}_finished.xlsx`;
