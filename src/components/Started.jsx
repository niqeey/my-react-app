...
const Started = () => {
    ...
    useEffect(() => {
        const fetchStartedParticipants = async () => {
            try {
                console.log('Fetching started participants for eventId:', eventId);
                const response = await authFetch(`${apiBase}/statistic/startlist`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ eventId }) // Use eventId from the URL
                });
                const result = await response.json();
                console.log('Started response:', result);
                console.log('Is array?', Array.isArray(result), 'Length:', result?.length);
                setData(result);
            } catch (error) {
                console.error('Error fetching started participants:', error);
            } finally {
                setLoading(false);
            }
        };
        ...
        if (eventId) {
            fetchStartedParticipants(); // Call the function only if eventId is available
        }
    ...
    return (
        <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 16 }}>
                <h2>Started Participants</h2>
                <button
                    onClick={async () => {
                        try {
                            const res = await authFetch(`${apiBase}/report/event/started/xlsx`, {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                            ...
                            a.href = url;
                            a.download = `${eventName || 'event'}_started.xlsx`;
